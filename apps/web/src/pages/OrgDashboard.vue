...
const loadSnapshot = async () => {
  ...
  try {
    const [membersRes, subjectsRes, questionsRes, quizzesRes] = await Promise.all([
      http.get<MemberResponse>(`/organizations/${orgId}/members`, { params: { limit: 1 } }),
      http.get<SubjectSummary[]>('/subjects', { params: { organization_id: orgId } }),
...
const popularSubjects = computed(() => subjects.value.slice(0, 4))
</script>
  ...
  <section class="space-y-8">
    ...
    <template v-else>
      <div class="grid gap-5 lg:grid-cols-4">
        ...
        <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <p class="text-xs font-semibold uppercase tracking-widest text-slate-400">Subjects</p>
          <p class="mt-3 text-3xl font-semibold text-slate-900">{{ subjects.length }}</p>
      ...
      <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <header class="flex items-center justify-between">
          <div>
            <h2 class="text-lg font-semibold text-slate-900">Subjects at a glance</h2>
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          >
            <div class="space-y-1">
              ...
              <p class="text-xs text-slate-500">{{ subject.description || 'Add a description to inspire learners.' }}</p>
            </div>
            <RouterLink
              :to="{ name: 'admin-questions', query: { subject: subject.slug } }"
          ...
          <div v-if="popularSubjects.length === 0" class="col-span-full rounded-2xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500">
            No subjects yet. Create a subject track to get started.
          </div>
        </div>
      </section>
    </template>
